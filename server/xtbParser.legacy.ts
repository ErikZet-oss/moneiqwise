...
export type XTBExportFormat = "legacy" | "v2";

export interface XTBOpenPositionLot {
  positionId: string;
  ticker: string;
  shares: number;
  openPrice: number;
...
export interface XTBOpenPositionsSnapshot {
  asOf: Date | null;
  totals: Array<{ product: string; value: number; profit: number; currency: string }>;
  lots: XTBOpenPositionLot[];
  parents: Array<{ ticker: string; shares: number; value: number; netProfit: number; netProfitPct: number; companyName?: string }>;
  reconciliation?: {
...
export interface XTBImportResult {
  ...
  format?: XTBExportFormat;
  openPositions?: XTBOpenPositionsSnapshot;
}
...
export async function parseXTBFileLegacy(fileBuffer: Buffer, _fileName: string): Promise<XTBImportResult> {
