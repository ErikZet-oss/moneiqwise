...
// Parse CASH OPERATION HISTORY / Cash Operations sheet
function parseCashOperations(
  data: any[][],
  log: ImportLogEntry[],
  accountCurrencyOverride?: string,
): ParsedTransaction[] {
  const transactions: ParsedTransaction[] = [];
  const accountCurrency =
    accountCurrencyOverride || parseAccountCurrencyFromXtBMeta(data);
  ...
  for (let i = headerIndex + 1; i < data.length; i++) {
    ...
    // Súhrnný riadok Total na konci v2 exportu (často bez Time)
    if (typePlain === 'total' || typeStr === 'total') {
      log.push({
        row: i + 1,
        status: 'skipped',
        message: `[${operationId}] Preskočené (Total)`,
      });
      continue;
    }
    ...
    // TAX
      ...
      // Poplatky z XTB (SEC fee, Stamp duty, Swap, Commission, …):
      ...
      const forex = buildForexForXtBLine(accountCurrency, row, fxCols, absAmt, lineSign);
      const base = Number.isFinite(forex.baseCurrencyAmount) ? forex.baseCurrencyAmount : lineSign * absAmt;
      const feeLabel = typeStr.includes('stamp duty')
        ? 'Stamp duty (XTB)'
        : typeStr.includes('swap')
          ? 'Swap (XTB)'
          : 'Poplatok brokera (XTB)';
      if (ticker) {
        transactions.push({
          date: time,
          ticker,
      ...
      if (!quantity || quantity <= 0 || !pricePerShare || !Number.isFinite(pricePerShare)) {
        log.push({
          row: i + 1,
          status: 'warning',
          message: `[${operationId}] ${typeStr}: nepodarilo sa určiť množstvo/cenu — ${comment.slice(0, 80)}`,
        });
        continue;
      }
      const amountForFx = totalAmount > 0 ? totalAmount : quantity * pricePerShare;
      const tradeFx = buildForexForXtBLine(
        accountCurrency,
        row,
        fxCols,
        amountForFx,
      ...
      transactions.push({
        ...
        quantity,
        priceEur: pricePerShare,
        totalAmountEur: amountForFx,
        originalComment: comment,
        externalId: operationId,
        originalCurrency: tradeFx.originalCurrency,
        exchangeRateAtTransaction: tradeFx.exchangeRateAtTransaction,
        baseCurrencyAmount: tradeFx.baseCurrencyAmount,
        instrumentPricePerShare,
        companyName: typeStr.trim(),
      });
      log.push({
        row: i + 1,
        status: 'success',
        message: `[${operationId}] ${isBuy ? 'BUY' : 'SELL'} ${quantity} ${ticker} @ ${pricePerShare.toFixed(2)} (${typeStr})`,
      });
...
function resolveAccountCurrency(
  openPositionsData: any[][] | null,
  fileName: string,
): string {
  if (openPositionsData) {
    for (let i = 0; i < Math.min(20, openPositionsData.length); i++) {
      const row = openPositionsData[i];
      if (!row) continue;
      const cells = row.map((c) => String(c ?? '').trim().toLowerCase());
      if (cells.includes('product') && cells.includes('metric') && cells.includes('currency')) {
        const ccyIdx = cells.indexOf('currency');
        for (let j = i + 1; j < Math.min(i + 10, openPositionsData.length); j++) {
          const code = String(openPositionsData[j]?.[ccyIdx] ?? '')
            .trim()
            .toUpperCase();
          if (/^[A-Z]{3}$/.test(code)) return code;
        }
  ...
  const m = base.match(/^(EUR|USD|GBP|CZK|PLN)_/i);
  if (m) return m[1].toUpperCase();

  return 'EUR';
}

function parseOpenPositionsSheet(data: any[][], log: ImportLogEntry[]): XTBOpenPositionsSnapshot {
  let asOf: Date | null = null;
  for (let i = 0; i < Math.min(15, data.length); i++) {
    const row = data[i];
    if (!row) continue;
    const first = String(row[0] ?? '').toLowerCase();
    if (first.includes('data as of report generated')) {
      asOf = parseDate(row[1]);
      break;
    }
  }

  const totalsMap = new Map<string, { product: string; value: number; profit: number; currency: string }>();
  for (let i = 0; i < Math.min(30, data.length); i++) {
    const row = data[i];
    if (!row) continue;
    const cells = row.map((c) => String(c ?? '').trim().toLowerCase());
    if (cells[0] === 'product' && cells[1] === 'metric') {
      for (let j = i + 1; j < Math.min(i + 20, data.length); j++) {
        const r = data[j];
        if (!r || !r[0]) break;
        const product = String(r[0] ?? '').trim();
        const metric = String(r[1] ?? '').trim().toLowerCase();
        if (!product || product.toLowerCase() === 'note') break;
        if (metric !== 'value' && metric !== 'profit') continue;
        const amount = parseAmount(r[2]);
        const currency = readIso3CurrencyFromCell(r[3], 'EUR');
        const prev = totalsMap.get(product) || { product, value: 0, profit: 0, currency };
        if (metric === 'value') prev.value = amount;
        else prev.profit = amount;
        prev.currency = currency;
        totalsMap.set(product, prev);
      }
      break;
    }
  }
  ...
  const parents: XTBOpenPositionsSnapshot['parents'] = [];
  const lots: XTBOpenPositionLot[] = [];

  if (headerIndex === -1) {
    log.push({
      row: 0,
      status: 'warning',
      message: 'Open Positions: Nenašla sa hlavička (Instrument/Position, Ticker, Volume)',
    });
    return { asOf, totals: Array.from(totalsMap.values()), lots, parents };
  }

  const instrumentCol = getColumnIndex(headers, [
    'instrument/position',
    'instrument',
    'position',
  ]);
  const tickerCol = getColumnIndex(headers, ['ticker', 'symbol']);
  const typeCol = getColumnIndex(headers, ['type']);
  const volumeCol = getColumnIndex(headers, ['volume', 'shares']);
  const valueCol = getColumnIndex(headers, ['value']);
  ...
  if (instrumentCol === -1 || tickerCol === -1 || volumeCol === -1) {
    log.push({
      row: 0,
      status: 'warning',
      message: 'Open Positions: Chýbajú stĺpce Instrument/Position, Ticker alebo Volume',
    });
    return { asOf, totals: Array.from(totalsMap.values()), lots, parents };
  }

  let currentParentCompany: string | undefined;

  for (let i = headerIndex + 1; i < data.length; i++) {
    const row = data[i];
    if (!row || row.every((cell) => cell == null || String(cell).trim() === '')) continue;

    const instrument = String(row[instrumentCol] ?? '').trim();
    const tickerRaw = String(row[tickerCol] ?? '').trim();
    const ticker = cleanTicker(tickerRaw);
    if (!instrument || !ticker) continue;

    const typeStr = typeCol >= 0 ? String(row[typeCol] ?? '').trim() : '';
    const volume = parseAmount(row[volumeCol]);
    const value = valueCol >= 0 ? parseAmount(row[valueCol]) : 0;
    const openPrice = openPriceCol >= 0 ? parseAmount(row[openPriceCol]) : 0;
    const currentRaw = currentPriceCol >= 0 ? row[currentPriceCol] : null;
    const currentPrice =
      currentRaw == null || String(currentRaw).trim() === ''
        ? null
        : parseAmount(currentRaw);
    const netProfit = netProfitCol >= 0 ? parseAmount(row[netProfitCol]) : 0;
    const netProfitPct = netProfitPctCol >= 0 ? parseAmount(row[netProfitPctCol]) : 0;
    const openTime = openTimeCol >= 0 ? parseDate(row[openTimeCol]) : null;
    const category = categoryCol >= 0 ? String(row[categoryCol] ?? '').trim() : '';

    const isNumericPosition = /^\d+$/.test(instrument);
    const isChildLot = /^buy$/i.test(typeStr) || isNumericPosition;
    const isParent = !typeStr && !isNumericPosition;

    if (isParent || (!isChildLot && category)) {
      currentParentCompany = instrument;
      parents.push({
        ticker,
        shares: volume,
        value,
    ...
    if (isChildLot) {
      lots.push({
        ...
        netProfitPct,
        openTime,
        companyName: currentParentCompany,
      });
    }
  }
  ...
  log.push({
    row: 0,
    status: 'success',
    message: `Open Positions: ${parents.length} parent, ${lots.length} lotov`,
  });

  return { asOf, totals: Array.from(totalsMap.values()), lots, parents };
}

function reconcileOpenPositions(
  snapshot: XTBOpenPositionsSnapshot,
  transactions: ParsedTransaction[],
): void {
  const importedByTicker = new Map<string, number>();
  for (const tx of transactions) {
    if (tx.type !== 'BUY' && tx.type !== 'SELL') continue;
    const prev = importedByTicker.get(tx.ticker) || 0;
    importedByTicker.set(
      tx.ticker,
      prev + (tx.type === 'BUY' ? tx.quantity : -tx.quantity),
    );
  }

  const shareDiffs: Array<{ ticker: string; imported: number; xtb: number }> = [];
  let tickersMatched = 0;

  for (const parent of snapshot.parents) {
    const imported = importedByTicker.get(parent.ticker) ?? 0;
    const xtb = parent.shares;
    const diff = imported - xtb;
    if (Math.abs(diff) > 1e-6) {
      shareDiffs.push({ ticker: parent.ticker, imported, xtb });
    } else {
      tickersMatched += 1;
    }
  }

  snapshot.reconciliation = { tickersMatched, shareDiffs };
}

/** Find Cash Operations sheet — never fall back to first sheet (Closed Positions). */
function findCashOperationsSheetName(sheetNames: string[]): string | null {
  const sheetNorm = (s: string) => stripDiacritics(s.toLowerCase());
  return (
    sheetNames.find((name) => {
      const n = sheetNorm(name);
      return n.includes('cash') && n.includes('oper');
    }) || null
  );
}

function findOpenPositionsSheetName(sheetNames: string[]): string | null {
  const sheetNorm = (s: string) => stripDiacritics(s.toLowerCase());
  return (
    sheetNames.find((name) => {
      const n = sheetNorm(name);
      return n.includes('open') && n.includes('position');
    }) || null
  );
}

// Main v2 parsing function
export async function parseXTBFileV2(
  fileBuffer: Buffer,
  fileName: string,
  ...
  try {
    ...
    const openSheetName = findOpenPositionsSheetName(workbook.SheetNames);
    let openPositionsData: any[][] | null = null;
    if (openSheetName) {
      openPositionsData = XLSX.utils.sheet_to_json(workbook.Sheets[openSheetName], {
        header: 1,
        raw: true,
      }) as any[][];
    }

    const accountCurrency = resolveAccountCurrency(openPositionsData, fileName);
    log.push({
      row: 0,
      status: 'success',
      message: `Mena účtu: ${accountCurrency}`,
    });

    const cashSheet = findCashOperationsSheetName(workbook.SheetNames);
    if (!cashSheet) {
      log.push({
        row: 0,
        status: 'error',
        message:
          'Hárok „Cash Operations“ nenájdený — import transakcií nie je možný (nepoužívam Closed Positions).',
      });
    } else {
      ...
      const cashTransactions = parseCashOperations(data, log, accountCurrency);
      transactions.push(...cashTransactions);
    }

    let openPositions: XTBOpenPositionsSnapshot | undefined;
    if (openPositionsData) {
      openPositions = parseOpenPositionsSheet(openPositionsData, log);
      reconcileOpenPositions(openPositions, transactions);
      const diffs = openPositions.reconciliation?.shareDiffs.length ?? 0;
      if (diffs > 0) {
        log.push({
          row: 0,
          status: 'warning',
          message: `Rekonciliácia: ${diffs} tickerov s rozdielom počtu akcií voči Open Positions`,
        });
      }
    } else {
      log.push({
        row: 0,
        status: 'warning',
        message: 'Hárok Open Positions nenájdený — snapshot portfólia nie je dostupný',
      });
    }
